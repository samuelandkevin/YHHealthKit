...
class Skoal: NSObject {
    ...
    func readBodyTemperatureFromHealthStore(completion:@escaping (_ value:Double,_ error:Error?) -> Void) -> Void {
        ...
        let endSort = NSSortDescriptor.init(key: HKSampleSortIdentifierEndDate, ascending: false)
        let predicate = self.predicateSampleByLatestData()
        let query = HKSampleQuery.init(sampleType: bodyTemperatureType!, predicate: nil, limit: 0, sortDescriptors: nil) { (query, results, error) in
...
//        let HKWasUserEntered = obj.metadata?["HKWasUserEntered"]
        ...
        let json:[String:Any] = ["uuid":uuid,"sourceRevision":sourceRevision,"device":device ?? [:],"metadata":metadata ?? [:],"sampleType":sampleType,"startDate":startDate,"endDate":endDate]
//        do {
//            let data = try JSON(obj).rawData()
