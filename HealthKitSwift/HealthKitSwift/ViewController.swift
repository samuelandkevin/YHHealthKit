...
class ViewController: UIViewController,UITableViewDelegate,UITableViewDataSource {
    ...
    let cervicalMucusQuality:[String] = ["宫颈黏液质量"]
    let ovulationTestResult:[String] = ["读取排卵测试结果","写入排卵测试结果"]
    let BBTResult:[String] = ["读取BBT","写入BBT"]
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell{
        ...
        if indexPath.section == 0 {
            ...
            cell.textLabel!.text = cervicalMucusQuality[indexPath.row]
        } else if indexPath.section == 23 {
            cell.textLabel!.text = ovulationTestResult[indexPath.row]
        } else if indexPath.section == 24 {
            cell.textLabel!.text = BBTResult[indexPath.row]
        }
    ...
    func tableView(_ tableView: UITableView, didSelectRowAt indexPath: IndexPath) {
        ...
        case 19:    // MenstrualFlow(月经)
            if indexPath.row == 0 {
                EHHealthKitCenter.sharedCenter.readMenstrualFlowFromHealthStore { (value, error) in
                    print("月经:\(String(describing: value)) \(String(describing: error))")
                }
            }
        case 20:    // IntermenstrualBleeding(点滴出血)
            if indexPath.row == 0 {
                EHHealthKitCenter.sharedCenter.readIntermenstrualBleedingFromHealthStore { (value, error) in
...
//                }
        ...
        case 21:    // SexualActivity(性行为)
            if indexPath.row == 0 {
                EHHealthKitCenter.sharedCenter.readSexualActivityFromHealthStore { (value, error) in
                    print("性行为:\(String(describing: value)) \(String(describing: error))")
                }
            }
        case 22:
            if indexPath.row == 0 {//宫颈黏液
                EHHealthKitCenter.sharedCenter.readCervicalMucusQualityFromHealthStore { (value, error) in
                    
                }
            }
        case 23: //排卵测试结果
            if indexPath.row == 0 {
                EHHealthKitCenter.sharedCenter.readOvulationTestResultFromHealthStore { (value, error) in
                    
                }
            }else if indexPath.row == 1 {
                EHHealthKitCenter.sharedCenter.writeOvulationTestResultToHealthStore(ovulationTestResult: 1) { (success, error) in
                    if success{
                        print("写入排卵测试结果到heathkit成功")
                    }else{
                        print("写入排卵测试结果到heathkit失败")
                    }
                }
            }
        case 24 ://BBT
            
            EHHealthKitCenter.sharedCenter.requestBBTAuthorization()
            
            if indexPath.row == 0 {
                EHHealthKitCenter.sharedCenter.readBBTFromHealthStore { (value, error) in
                    print("\(value)\n\(error.debugDescription)")
                }
            }else if indexPath.row == 1 {
                EHHealthKitCenter.sharedCenter.writeBBTToHealthStore(bodyTemperature: 36.5) { (success, error) in
                    print("\(success)\n\(error.debugDescription)")
                }
            }
